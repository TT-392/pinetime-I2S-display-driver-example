...
static inline void transfer_init() {
    I2S_enable(1);
...
static inline void transfer_cleanup() {
...
void I2S_RAMWR_bitmap(uint8_t* data, int pixCount)  {
    ...
    int buffSize = 16; // has to be a multiple of 4, higher value = more stable but also more memory
...
void I2S_RAMWR_solid_color(uint16_t color, int pixCount) {
    ...
    setup_next_txptr(start, sizeof(start));
    NRF_I2S->TASKS_START = 1;
